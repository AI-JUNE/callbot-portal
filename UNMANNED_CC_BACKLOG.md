...
3. **시나리오 빌더(간이)** ✅(2026-07-30 확인 — public/admin.html view-builder에 노드 추가·편집·삭제, 대본 단계 편집, 검증·JSON 내보내기·버전 저장, 배치 테스트 기 구현. 데이터 로컬/데모) — 관리자 콘솔(admin.html)에서 인텐트→응답→분기 편집 UI. 데이터는 로컬/데모.
